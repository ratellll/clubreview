...
//
//     const initMap = async () => {
//         if (isMapInitializing || isMapReadyRef.current) {
...
//     };
//
//     // ESC 키로 인포윈도우 닫기를 위한 전역 이벤트 리스너
//     useEffect(() => {
//         const handleGlobalEscKey = (e) => {
//             if (e.key === 'Escape' && currentInfoWindowRef.current) {
//                 console.log('ESC key pressed - closing info window');
//                 currentInfoWindowRef.current.close();
//                 setCurrentInfoWindow(null);
//                 currentInfoWindowRef.current = null;
//                 // 모든 마커의 상태 업데이트
...
//                         console.log('ESC - marker state reset:', m.clubName);
//                         m.isInfoWindowOpen = false;
//                     }
//                 });
//             }
...
//                 // 인포윈도우 닫기
//                 if (currentInfoWindowRef.current) {
//                     console.log('Closing info window with ESC');
//                     currentInfoWindowRef.current.close();
//                     setCurrentInfoWindow(null);
//                     currentInfoWindowRef.current = null;
//
...
//                 const existingModal = document.querySelector('.photo-modal');
//                 if (existingModal) {
//                     console.log('Closing photo modal with ESC');
//                     existingModal.remove();
//                 }
//             }
//         };
...
//     useEffect(() => {
//         if (location.state?.refresh) {
//             console.log('Page refresh - reloading map and data');
//             resetMapAndData();
//             window.history.replaceState(null, '');
//         }
//     }, [location.state?.refresh]);
...
//
//         if (mapInstanceRef.current && clubs.length > 0 && isMapReadyRef.current) {
//             console.log('All conditions met - calling displayMarkersOnMap from useEffect');
//             displayMarkersOnMap();
//         } else {
//             console.log('Conditions not met for marker display in useEffect');
//         }
//     }, [clubs]);
//
//     const initializeMap = () => {
...
//
//         if (mapInstanceRef.current) {
//             console.log('Map already exists - stopping initialization');
//             return;
//         }
//         if (!mapRef.current) {
...
//
//             setTimeout(() => {
//                 console.log('Map relayout and marker check');
//                 kakaoMap.relayout();
//
//                 // 지도 생성 완료 후 clubs 데이터가 있으면 마커 표시
//                 if (clubs.length > 0) {
//                     console.log('Map ready, clubs available - calling displayMarkersOnMap from initializeMap');
//                     console.log('Clubs data in initializeMap:', clubs);
//                     displayMarkersOnMap();
//                 } else {
//                     console.log('No clubs data available yet in initializeMap');
//                 }
//             }, 300);
//
//             // 지도 클릭 시 정보창 닫기
//             window.kakao.maps.event.addListener(kakaoMap, 'click', () => {
//                 console.log('Map background clicked - closing info window');
//
//                 if (currentInfoWindowRef.current) {
//                     currentInfoWindowRef.current.close();
//                     setCurrentInfoWindow(null);
...
//             // 클럽 데이터 로드 완료 후 지도가 준비되어 있으면 마커 표시
//             if (mapInstanceRef.current && isMapReadyRef.current) {
//                 console.log('Map ready, calling displayMarkersOnMap from fetchClubs');
//                 displayMarkersOnMap();
//             } else {
//                 console.log('Map not ready in fetchClubs');
//             }
//         } catch (err) {
...
//
//     const displayMarkersOnMap = () => {
//         console.log('displayMarkersOnMap function started');
//
//         if (!window.kakao || !window.kakao.maps || !mapInstanceRef.current) {
//             console.log('Missing dependencies for marker display');
//             return;
//         }
//
//         console.log('=== MARKER DISPLAY START ===');
...
//
//             console.log(`Club coordinates - lat: ${lat}, lng: ${lng}`);
//             console.log(`Finite check - lat: ${Number.isFinite(lat)}, lng: ${Number.isFinite(lng)}`);
//
//             if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
//                 console.log(`Skipping ${club.name} - invalid coordinates (lat: ${club.latitude}, lng: ${club.longitude})`);
//                 return;
//             }
//
//             try {
//                 console.log(`Creating marker for ${club.name} at (${lat}, ${lng})`);
//
...
//                 marker.infoWindow = infoWindow;
//                 marker.isInfoWindowOpen = false;
//                 marker.clubName = club.name;
//
//                 // 마커 클릭 이벤트
//                 window.kakao.maps.event.addListener(marker, 'click', () => {
//                     console.log('Marker clicked:', club.name);
//
//                     if (marker.isInfoWindowOpen && currentInfoWindowRef.current === infoWindow) {
//                         // 같은 마커 재클릭 - 정보창 닫기
//                         console.log('Same marker re-clicked - closing info window');
//                         infoWindow.close();
//                         setCurrentInfoWindow(null);
//                         currentInfoWindowRef.current = null;
//                         marker.isInfoWindowOpen = false;
//                     } else {
//                         // 다른 마커 클릭 또는 정보창이 닫혀있는 경우
//
//                         // 먼저 현재 열려있는 정보창 닫기
//                         if (currentInfoWindowRef.current) {
...
//
//                         // 새로운 정보창 열기
//                         console.log('New info window opened:', club.name);
//                         infoWindow.open(mapInstanceRef.current, marker);
//                         setCurrentInfoWindow(infoWindow);
//                         currentInfoWindowRef.current = infoWindow;
//                         marker.isInfoWindowOpen = true;
...
//         });
//
//         // 마커가 있으면 지도 범위를 마커들에 맞춤
//         if (validMarkerCount > 0) {
//             console.log(`Fitting map bounds to ${validMarkerCount} markers`);
